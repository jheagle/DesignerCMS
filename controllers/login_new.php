    ...
    if (empty($_SESSION['username']) || trim($_SESSION['username']) === '') {
        ...
        if ((!empty($_POST['username']) || trim(
            ...
            $result = $db->select_assoc(
              "SELECT id FROM {$table} WHERE access_email='{$email}'"
            );
            $count = count($result);
            if ($count) {
            ...
            if (!$fail) {
                $result = $db->insert(
                  "INSERT INTO account (username, password) VALUES('{$user}', SHA2('{$pass}', 512))"
                );
                $accountId = $db->lastInsertId();
                $result = $db->insert(
                  "INSERT INTO {$table} (username, password, email) VALUES('{$user}', SHA2('{$pass}', 512), '{$email}')"
                );
                if ($result) {
