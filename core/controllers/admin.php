    ...
    if (count($_POST) < 1) {
        ...
        if (isset($_GET["filter"])) {
            ...
            if ($filter != "" && $filter != "all" && $filter[0] != "all") {
                ...
                $query = sprintf("SELECT * FROM %s %s ORDER BY name", $table,
                  $s);
            }
        }
        ...
        $telephone = isset($_POST["tel"]) ? sanitize(
          preg_replace('/\D+/', '', $_POST["tel"])
        ) : '';
        ...
        if (!empty($_POST["id"])) {
            if (isset($_POST['create'])) {
                // Search the rows in the markers table
                $query = sprintf(
                  "UPDATE %s SET name='%s', lat=%f, lng=%f, contact='%s', address='%s', tel=%s, link='%s', product='%s', info='%s' WHERE id=%d",
                  $table,
                  $location_name,
                  $latitude,
                  $longitude,
            ...
            if (isset($_POST['create'])) {
                // Search the rows in the markers table
                $query = sprintf(
                  "INSERT INTO %s (name, lat, lng, contact, address, tel, link, product, info) VALUES ('%s', %f, %f, '%s', '%s', '%s', '%s', '%s', '%s')",
                  $table,
                  $location_name,
                  $latitude,
                  $longitude,
...
function parseGetArray($input)
{
    ...
    if (is_array($input)) {
        foreach ($input as $val) {
            ...
            $queryString .= $cnt > 0 ? sprintf(
              " AND FIND_IN_SET('%s',  REPLACE(product, ', ', ','))",
              $val
            ) : sprintf("FIND_IN_SET('%s', REPLACE(product, ', ', ','))", $val);
            $cnt++;
        }
    } else {
        $queryString = "";
